...
class ExchangeCard {
  ...
  render(container, ctx = {}, isBest = false) {
    ...
    this._el.innerHTML =
      ...
      `<div class="top"><span class="name" style="background:var(--${this.key},${m.color});color:var(--badge-ink);font-size:10px;font-weight:700;padding:2px 8px;border-radius:4px;letter-spacing:.4px;">${escapeHtml(m.label)}</span>` +
      `<svg class="spark" viewBox="0 0 34 16"><polyline fill="none" style="stroke:var(--${this.key},${m.color})" stroke-width="1.3" points="0,12 6,8 12,10 18,4 24,7 30,3 34,5"/></svg></div>` +
